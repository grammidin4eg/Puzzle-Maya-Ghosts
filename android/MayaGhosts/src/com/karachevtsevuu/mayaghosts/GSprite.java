...
public class GSprite extends AnimatedSprite {
	...
	private void goAway() {		
		...
		final float ddt_y = -1*xmove*MainActivity.screenGrid/3+(ymove>0 ? -1*this.getHeightScaled()/2: 0)-MainActivity.screenMargin;
		final float ddt_x = -1*xmove*MainActivity.screenGrid/3/*+(xmove>0 ? -1*this.getWidthScaled()/2: 0)*/;
		final float ax = Math.round((this.getX()+ddt_x)/MainActivity.screenGrid);
		final float ay = Math.round((this.getY()+ddt_y)/MainActivity.screenGrid);
