...
public class MainGrid {
	...
	private void createBoom(final float boom_x, final float boom_y) {
		...
		MainActivity.mainActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				boom.animate(5, false, new IAnimationListener() {//50
					...
					public void onAnimationFinished(final AnimatedSprite pAnimatedSprite) {									
						for (GBlock cur_block : mBlocks) {
							if (pAnimatedSprite.collidesWith(cur_block.getSprite())) {
								if(cur_block.isBombBlock()) {
									Sprite curs = cur_block.getSprite();
									createBoom(curs.getX()+curs.getWidthScaled()/2,curs.getY()+curs.getHeightScaled()/2);
								}
								cur_block.remove();								
								break;
							}
	...
	public void setGraphObject(Room room) {
		...
		for(int xx = 0;xx<10;xx++) {	
			for(int yy = 0;yy<10;yy++) {
				...
				case 8:
					...
					GBlock newBombBlock = new GBlock(xx,yy);
					newBombBlock.setSprite(bomb);
					newBombBlock.setButton(new GButton(bomb, bomb_active));
					newBombBlock.setAction(new IGameAction() {
						
						@Override
						public boolean RunCollision(final Sprite self, final GButton button) {
							...
							final TimerHandler spriteTimerHandler = new TimerHandler(1, new ITimerCallback() {
										...
										public void onTimePassed(
											...
											if( mStep<7 ) {
												...
												final float boom_x = self.getX()+self.getWidthScaled()/2;
												final float boom_y = self.getY()-self.getHeightScaled()/2;
				...
				case 11://бонус
					...
					GBlock newBonusBlock = new GBlock(xx,yy);
				...
				case 7://звезда
					...
					GBlock newBlock = new GBlock(xx,yy);
				...
				case 5://стена
					...
					GBlock newBlockWall = new GBlock(xx,yy);
				...
				case 3:
					if( MainActivity.mainActivity.sBall!=null) {
						...
						GBlock newPLBlock = new GBlock(xx,yy);
						newPLBlock.setSprite(face);
						newPLBlock.setMovedBlock(true);
						blocks.add(newPLBlock);
					}
					break; 
		...
		mBlocks = blocks;
		MatrixMover.getSelf().setBlocks(blocks);
